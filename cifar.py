...
memory = torch.zeros((args.low_dim, ndata), requires_grad=False).cuda()
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets, indexes) in enumerate(trainloader):
        ...
        outputs_new = torch.mm(features, memory.detach().float()).div(0.1)
        loss = criterion(outputs_new, indexes)

        loss.backward()
        optimizer.step()

        with torch.no_grad():
            memory[:, indexes] = features.t()
...
for epoch in range(start_epoch, start_epoch+200):
    train(epoch)
    acc = kNN(epoch, net, memory, trainloader, testloader, 200, args.nce_t, 0)
    acc_saver.append(acc)
    ...
    np.save('acc_projected.npy', np.asarray(acc_saver))


acc = kNN(0, net, memory, trainloader, testloader, 200, args.nce_t, 1)
